...
intents.message_content = True  # CRITICAL: Needs toggle turned ON in Developer Portal
...
def get_user_profile(data, user_id):
    if user_id not in data:
        ...
        old_points = data[user_id]
        data[user_id] = {"points": old_points, "inventory": []}
        save_data(data)
...
async def on_message(message):
    ...
    # Dual-verification: matches if ID matches OR if the name matches
    is_spam_channel = (message.channel.id == SPAM_CHANNEL_ID) or (getattr(message.channel, 'name', '') == SPAM_CHANNEL_NAME)
    
    if is_spam_channel and content_clean == "lalamove":
        all_data = load_data()
        ...
        try:
            ...
            print(f"ERROR: Bot missing 'Add Reactions' permission.")
