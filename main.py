...
async def on_message(message):
    ...
    # Custom keyword tracking group matrix matches
    allowed_triggers = ("lalamove", "lala", "lalamo", "lala mo")
    if message.content.strip().lower() in allowed_triggers:
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send(f"❌ **Missing argument!** Correct syntax: `{ctx.prefix}{ctx.command.name} {ctx.command.signature}`")
    elif isinstance(error, commands.BadArgument):
        await ctx.send("❌ **Invalid data type!** Make sure your target user is mentioned properly and numeric values are normal digits.")
    elif isinstance(error, commands.CommandNotFound):
        pass  
    else:
        print(f"Railway Log Exception Tracker: {error}")

@bot.command(name="lalahelp")
async def lalahelp(ctx):
    embed = discord.Embed(
        title="📦 Lalamove Economy Logistics Manual", 
        description="Welcome to the ultimate delivery grind! Type **`lalamove`**, **`lala`**, **`lalamo`**, or **`lala mo`** anywhere to earn base logistics points.",
        color=discord.Color.orange()
    )
    embed.add_field(name="⚙️ Core Command Hub", value=(
        "`!lalahelp` - Display this operation layout window.\n"
        "`!lalapoints [@user]` - Check logistics net worth stats.\n"
        "`!lalaowned [@user]` - Review garage items and active multipliers.\n"
        "`!lalatop` - View top 5 ranking system drivers."
    ), inline=False)
    embed.add_field(name="🛒 Fleet Capital Markets", value=(
        "`!lalashop` - Open showroom scaling grid upgrades.\n"
        "`!lalabuy [item]` - Purchase multiplier items (e.g. `!lalabuy lalamove van`).\n"
        "`!lalasendpoints [@user] [amount]` - Wire points safely to another driver."
    ), inline=False)
    embed.set_footer(text="Multi-Server Economy Version 2.5 | Powered by Railway")
...
async def lalapoints(ctx, member: discord.Member = None):
    ...
    await ctx.send(f"📦 {target.mention} has **{profile['points']:,}** points!")
...
async def lalabuy(ctx, *, item_name: str):
    ...
    if profile["points"] < item["price"]: 
        ...
        return await ctx.send(f"❌ {ctx.author.mention}, you need `{shortage:,}` more points to purchase **{item['display_name']}**!")
...
async def lalasendpoints(ctx, member: discord.Member, amount: int):
    ...
    if sender["points"] < amount: 
        return await ctx.send(f"❌ {ctx.author.mention}, you have insufficient points!")
        
    sender["points"] -= amount
    receiver["points"] += amount
    save_data(data)
    await ctx.send(f"💸 {ctx.author.mention} sent `{amount:,}` points to {member.mention}!")
