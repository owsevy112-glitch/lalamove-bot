...
async def on_message(message):
    ...
    if message.channel.id == SPAM_CHANNEL_ID and content_clean == "lalamove":
        ...
        # Calculate total bonus multipliers from fleet
        total_bonus = 0
...
async def lalaowned(ctx, member: discord.Member = None):
    ...
    for item_id, info in SHOP_ITEMS.items():
        ...
        if qty > 0:
            fleet_list += f"✅ {emoji} **{name}** — *Count: {qty}* (+{qty * info['multiplier']} mult)\n"
            total_vehicles += qty
        else:
    ...
    embed.add_field(name="Total Fleet Size", value=f"`{total_vehicles}` active vehicles", inline=True)
...
async def lalashop(ctx):
    ...
    for item_id, info in SHOP_ITEMS.items():
        embed.add_field(
            ...
            value=f"Cost: `{info['price']}` points | Bonus: `+ {info['multiplier']}` per message",
            inline=False
        )
...
async def lalabuy(ctx, *, item_name: str = None):
    if not item_name:
        await ctx.send(f"{ctx.author.mention}, please tell me what you want to buy! Example: `!lalabuy lalamove bike`")
        return
...
async def lalatop(ctx):
    ...
    # Blockquote added right before the footer notes
