...
async def on_message(message):
    ...
    # 1. Earn points loop: Checks the specific channel and exact phrase match
    if message.channel.name == SPAM_CHANNEL_NAME and content_clean == "lalamove":
        all_data = load_data()
        ...
        # Add visual reaction to confirm it registered
        try:
            await message.add_reaction("📦")
        except discord.Forbidden:
            print(f"ERROR: Bot lacks 'Add Reactions' permission in #{message.channel.name}")
        except discord.HTTPException:
            pass
            
    # 2. Always processes commands for every single message so nothing is blocked
